if (-not (Test-Path "D:\trans\international_transport_app\lib\services")) { New-Item -ItemType Directory -Path "D:\trans\international_transport_app\lib\services" -Force }
$content = @'
import 'dart:io';
...
class ExcelService {
  ...
  Future<Uint8List> exportTreasuryTransactions(List<Map<String, dynamic>> transactions) async {
    ...
    final sheet = excel['سجلات_الخزينة'] ?? excel.createSheet('سجلات_الخزينة');
    ...
    for (var i = 0; i < transactions.length; i++) {
      ...
      sheet.cell(CellIndex.indexByString('${_columnLabel(2)}$rowStr')).value = amount;
      sheet.cell(CellIndex.indexByString('${_columnLabel(3)}$rowStr')).value = TextCellValue(description);
    }

    return Uint8List.view(excel.encode()!);
  }
  ...
  Future<Uint8List> exportInvoices(List<Map<String, dynamic>> invoices) async {
    ...
    final sheet = excel['الفواتير'] ?? excel.createSheet('الفواتير');
    ...
    for (var i = 0; i < invoices.length; i++) {
      ...
      sheet.cell(CellIndex.indexByString('${_columnLabel(4)}$rowStr')).value = totalAmount;
      sheet.cell(CellIndex.indexByString('${_columnLabel(5)}$rowStr')).value = paidAmount;
      sheet.cell(CellIndex.indexByString('${_columnLabel(6)}$rowStr')).value = remaining;
  ...
  Future<Uint8List> exportFinancialReport(Map<String, double> report) async {
    ...
    final sheet = excel['التقرير_المالي'] ?? excel.createSheet('التقرير_المالي');
    ...
    report.forEach((key, value) {
      ...
      sheet.cell(CellIndex.indexByString('${_columnLabel(1)}$rowStr')).value = value;
      row++;
    });
  ...
  Future<void> shareExcel(Uint8List bytes, String filename) async {
    ...
    await Share.shareXFiles([xFile], text: 'ملف Excel: $filename.xlsx');
  }
...
'@
Set-Content -Path "D:\trans\international_transport_app\lib\services\excel_service.dart" -Value $content -Encoding UTF8
